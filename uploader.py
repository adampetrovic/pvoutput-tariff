...
def send_price_to_pvoutput(api_key: str, system_id: str, extended_param: str, price: float, now: datetime) -> requests.Response:
...
@click.option('--timezone', 'timezone', envvar='TZ', default='Australia/Sydney', required=True, help='Timezone for calculations.')
def main(config_path: str, api_key: Optional[str], system_id: Optional[str], timezone: str):
    ...
    try:
        ...
        current_tariff = get_current_tariff(
            config.get('tariffs'), 
            config.get('public_holidays'), 
            current_datetime
        )
        
        response = send_price_to_pvoutput(
            api_key, 
            system_id, 
